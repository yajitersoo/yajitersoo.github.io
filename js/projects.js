document.addEventListener("DOMContentLoaded", () => {
  ...
  function clearViewerFinal() {
    if (!modalFrameFinal || !modalImageFinal) return;
  ...
  function openModalViewerFinal(card) {
    ...
    launchCardsFinal.forEach((item) => item.classList.remove("is-active"));
  ...
  document.addEventListener("keydown", (event) => {
    if (event.key === "Escape" && modalFinal && modalFinal.classList.contains("is-open")) {
      closeModalViewerFinal();
    }
  });
